...
export default () => {
  return <React.Fragment>
    ...
    <h2>Stateless / Stateful Component</h2>
    <hr />
    <Note>在兩個無狀態的組件互動相當有限，約略只能做初始值傳遞與function call back；當想要在資料變動(互動)時自動渲染到頁面(或子組件)上，如果直接去更改值這樣是無法讓頁面重新渲染的，因為 React 真的沒有這麼聰明，你一定要跟他說那些東西變動時要重新渲染畫面，而這個關鍵就是 state 狀態。</Note>
    <Father />
    <Note>當父層具有狀態 state 時，與無狀態子層互動時可以傳遞 state 進入子層作為 props，當 state 透過 setState() 改變後會自動渲染至子層，讓畫面重新刷新顯示新資料。如以下例點選 "Tell son another secret" 按鍵時，其實只是透過 setState() 修改父層內部狀態的值，而子層中因為有使用到父層傳遞進來的狀態，因此畫面也會一併刷新；反之亦然，當子層點選 "Tell father I'm so happy" 按鍵時，也只是透過 function call back 通知父層自己修改自己的內部狀態值，重新刷新畫面顯示新值。</Note>
    <FatherWithState />
  </React.Fragment>
}
